...
// State handling
export default function CertificateDashboardTable({
  ...
  return (
    <div className="flex flex-col justify-between  w-full overflow-x-auto">
      <div className="fixed z-50 right-8 animate-bounce">
        {showSuccessToast && (
          <SuccessToast
            text={`${successfulEmail} sent successfully. ${failedEmail} failed`}
          />
        )}
        {showFailureToast && (
          <FailureToast
            text={`${failedEmail} failed. ${successfulEmail} sent successfully. `}
          />
        )}
      </div>
      {loading2 && (
            ...
            allLearners.map((row, index) => (
              <tr key={row.id}>
                <td className="px-2 py-4 whitespace-nowrap text-[10px] text-tableText2 font-medium flex flex-row cursor-pointer">
                  <span className="pr-4">
                    <input
                      ...
                      onChange={() => row.certificate_status !== "Approved" && handleCheckboxChange(row.id)}
                      checked={row.certificate_status !== "Approved" && !!checkedItems[row.id]}
                    />
                  </span>
                  {row.name}
                </td>
                  ...
                  <p
                    className={clsx("text-center p-1 rounded-lg", {
                      "bg-green4 text-green3":
                        row.certificate_status === "Approved",
                ...
                <td className="px-2 py-4 whitespace-nowrap text-[9px] text-tableText2 font-bold text-center relative">
                  ...
                  {editOptionsVisible === index && row.certificate_status !== "Approved" && (
                    <div
                      ref={menuRef}
