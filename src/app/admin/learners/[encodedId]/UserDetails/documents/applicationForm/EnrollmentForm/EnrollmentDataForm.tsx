...
const getFieldEditability = (
  ...
  // Admin can never edit
  if (userRole === "Admin") return false;

  // After submission
  if (isSubmitted) {
    // Learner can't edit after submission
    if (userRole === "learner") return false;
    // Accessor can edit if they have permission
    if (userRole === "accessor") return editableBy.includes("accessor");
  }
  // Before submission
  else {
    // Learner can edit if they have permission
    if (userRole === "learner") return editableBy.includes("learner");
    // Accessor can't edit before submission
    if (userRole === "accessor") return false;
  }
    ...
    <div>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 my-4">
        {formFields.map((field, index) => {
          const isEditable = getFieldEditability(
            field.editableBy,
            userRole,
            isSubmitted
          );
