...
import { assignDocToLearners, deleteDocument, postURLToDb } from "../utils/action";
 ...
 */
const FilesInFolder = ({ folderId, files, error, loading, createFolder, refetchData }) => {
  ...
  const handleAddDocumentClick = async () => {
    const response = await assignDocToLearners({ documentId: selectedFileId, studentIds: selectedIds });
    setActionResponse(response === 201 ? "201" : "An error occurred, try again");
  ...
  return (
    <div className="flex flex-col py-2">
      {loading ? (
        ...
        <ul>
          {files.map((file) => (
            <div className="ml-10 flex-col flex md:flex-row space-y-2 justify-between my-3" key={file.id}>
              <div className="flex flex-row justify-start space-x-6">
                <Image src="/file_icon.png" alt="folder icon" height={10} width={30} />
                <p className="text-sm text-gray-700 cursor-pointer" onClick={() => handleFileClick(file.publicUrl)}>
                  {file.fileName}
                </p>
              </div>
              <div className="flex flex-row space-x-4 items-center mr-8">
                <MdOutlineDeleteForever className="text-red-600" onClick={() => openFileDeleteModal(file.id)} />
                <div className="group flex flex-row items-center space-x-2" onClick={() => assignFiles(file.id)}>
                  <MdOutlineCheckBoxOutlineBlank className="text-gray-700 group-hover:text-dashboardButtons" />
                  <p className="text-sm text-gray-700 group-hover:text-dashboardButtons">Assign document</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="flex flex-col justify-center items-center lg:flex-row gap-3">
        <Button className="bg-dashboardButtons" onClick={() => createFolder(folderId)}>
          Create Sub-Folder
        </Button>
        <UploadFileBtn onUploadClick={() => setIsUploadModalOpen(!isUploadModalOpen)} />
      </div>
      <DeleteFile openModal={openDeleteModal} onClose={() => setOpenDeleteModal(false)} onFileDeleteClick={handleFileDelete} loading={deleting} />
    </div>
  );
};
